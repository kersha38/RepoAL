...
def runSGOA():
    ...
    def abrirBusqueda():
        ...
        ventaBus = tk.Tk()
        ventaBus.geometry("300x300+200+200")
        ventaBus.title("Busqueda")
        ventaBus.configure(background="#38EB5C")
        lblcampo = tk.Label(ventaBus, text="Escoger campo").place(x=20, y=50)
        lstCamoo = tk.Listbox(ventaBus, bg="green", fg="#000", width=23, height=4)
        ...
        campo_text.pack()
        campo_text.get()
        # imagen2=PhotoImage(file = "buscar.gif")
        # lblimagen2 = Label(ventaBus, image=imagen2).place(x=400, y=10)
        ventaBus.mainloop()
    ...
    def abrirVentana():
        ...
        ventAV.configure(background="green")
        lblOpciones = tk.Label(ventAV, text="Escoger una opcion").place(x=50, y=50)
        select = tk.IntVar()
        rdBOP = tk.Radiobutton(ventAV, text="Importar y catalogar OA", command=intCargar.cargar, value=1,
                               variable=select).place(x=50, y=100)
        rdBOP = tk.Radiobutton(ventAV, text="Busqueda de OA", value=2, variable=select, command=abrirBusqueda).place(
            x=50, y=150)
        ventAV.mainloop()
    ...
    lblImagen = Label(ventana, image=imagen).place(x=400, y=10)
    ventana.mainloop()